...
The jump and gravity logic go hand in hand. In the `Jump()` method, I implemented a `transform.Translate` function that exists outside of the `if` statement that checks whether the player is grounded. This `transform.Translate` takes a `new Vector3` with its X and Z values set to 0 and the Y value set to a `velocity` float variable.
